...
export default function BetaSignup({ config }: { config: BrokerageConfig }) {
  ...
  useEffect(() => {
    const fetchCount = () =>
      fetch(`/api/beta/${config.slug}/count`)
        .then((r) => r.json())
  ...
  const onSubmit = async (data: FormData) => {
    setServerError('');
    const res = await fetch(`/api/beta/${config.slug}/signup`, {
      method: 'POST',
          ...
          <>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-gray-400 text-sm">
                Brokerage: <span className="text-white font-medium">{config.name}</span>
              </div>
              ...
              <p className="text-center text-xs text-gray-500">
                60 days to use it · No credit card · {config.name} agents only
              </p>
            </form>
          </>
        ) : (
          ...
          >
            ...
            {gotFreeDeal ? (
              ...
              <>
                ...
                <p className="text-xl text-gray-300">{config.waitlist_message}</p>
              </>
            )}
